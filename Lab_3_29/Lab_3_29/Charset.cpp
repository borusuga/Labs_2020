...
//перегрузка операторов
    ...
    CharSet* CharSet::operator+= (CharSet& charset){
        return this->Addition(charset);
    }

    CharSet CharSet::operator- (CharSet& charset){ //работает
        return *(this->Subtraction(charset));
    }


    CharSet CharSet::operator+ (CharSet& charset){   // работает
        int k = this->cardinality+charset.cardinality;
        char* nc = new char[k];
        for (int i = 0; i < k; ++i){
            nc[i] = '\0';
        }
        ...
        for (int i = 0; i < charset.cardinality; ++i){
            nc[this->cardinality + i] = charset.value[i];
        }
        nc = DeleteIdenticalChar(nc);
        ThirdLab3::CharSet ncs = ThirdLab3::CharSet(nc);
        delete [] nc;
        return ncs;
